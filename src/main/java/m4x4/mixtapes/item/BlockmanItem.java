...
import m4x4.mixtapes.item.inventory.BlockmanInventoryCapability;
...
public class BlockmanItem extends Item {
	public BlockmanItem() {
		super(new Item.Properties().tab(MaxsMixtapesModTabs.TAB_ITEM_TAB).stacksTo(1).rarity(Rarity.EPIC));
	}

	@Override
	public void appendHoverText(ItemStack itemstack, Level world, List<Component> list, TooltipFlag flag) {
		super.appendHoverText(itemstack, world, list, flag);
		list.add(Component.literal("\"I'll never forgive the Japanese, but I do love my walkman\""));
	}
		...
		if (entity instanceof ServerPlayer serverPlayer) {
			NetworkHooks.openScreen(serverPlayer, new MenuProvider() {
				...
				public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
					...
					return new BlockmanPlayerMenu(id, inventory, packetBuffer);
				}
			}, buf -> {
