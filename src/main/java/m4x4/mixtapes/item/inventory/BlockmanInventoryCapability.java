...
package m4x4.mixtapes.item.inventory;
...
public class BlockmanInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	...
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntity().getItem().getItem() == MaxsMixtapesModItems.BLOCKMAN.get()) {
			if (Minecraft.getInstance().screen instanceof BlockmanPlayerScreen) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(61) {
			@Override
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != MaxsMixtapesModItems.BLOCKMAN.get();
			}
